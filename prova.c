...
unsigned char frameCount = 0;  // para controlar espa�amento dos inimigos
unsigned char seed = 3;        // semente simples para pseudo-random
unsigned char morte = 0;       // 0 = parede, 1 = batida
unsigned int distancia = 0;    // em d�cimos de km (ex: 12 = 1.2km)
...
// === Inverte alguns caracteres que estavam sendo mostrados de ponta cabe�a ===
...
// === Atualiza carrinho com bot�es ===
void updateCarPosition() {
    static bit btnLeftPrev = 1;
    static bit btnRightPrev = 1;
		
		// === Precisa esperar o bot�o soltar para evitar debounce ===
...
void generateNewLine(unsigned char *newLine) {
    ...
    // Entra em modo t�nel a partir de 15.0 km
    if(distancia >= 150) {
        modoTunel = 1;
    }
}
...
void redrawTrack() {
    int i, j;
    unsigned char tempLinha8[10];	// O uso de array tempor�rio � necess�rio para mudan�as de fundo entre
																	// pista e t�nel sem gerar erros como deletar o carrinho do jogador ou
																	// outros problemas de atualiza��o.
    ...
    for(j = 0; j < 7; j++) {
        ...
        for(i = 0; i < 10; i++) {
            ...
            if (modoTunel) {
                ...
                // Carrinho inimigo invis�vel, exceto se estiver no cone do farol
                if (linha[i] == 4) {
                    int visivel = 0;
                    int dist = 7 - j; // dist�ncia at� o carrinho (linha 8)
...
void vitoriaScreen() {
    ...
    while(1);  // trava o jogo ap�s vit�ria
}
